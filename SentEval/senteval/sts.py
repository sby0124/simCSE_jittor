    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            ...
            for ii in range(0, len(gs_scores), params.batch_size):
                batch1 = input1[ii:ii + params.batch_size]#取出batch_size个句子
                batch2 = input2[ii:ii + params.batch_size]#取出batch_size个句子

                # we assume get_batch already throws out the faulty ones
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    enc1 = batcher(params, batch1)#将batch_size个句子转换为embedding,形状为(batch_size, embedding_size)
            ...
            #将两个写入文件
            # with open('sys_scores_train.txt', 'w') as f:
            #     for i in range(len(all_sys_scores)):
            #         f.write(str(i) + '\n')
            #         f.write(str(all_sys_scores[i]) + '\n')
            #         f.write(str(all_gs_scores[i]) + '\n')
            # input() 
